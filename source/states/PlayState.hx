...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'phantom-arcade': new states.stages.PhantomArcade(); //Week 2
			case 'phantom-arcade-evil': new states.stages.PhantomArcadeEvil(); //Week 2 Alt BG
		...
		if (OpenFlAssets.exists(Paths.json(SONG.song.toLowerCase() + "/meta"), TEXT))
		{
			meta = new SongMetaTags(0, 135, SONG.song.toLowerCase());
			meta.cameras = [camHUD];
			add(meta);
		}

		healthBar = new Bar(0, FlxG.height * (!ClientPrefs.data.downScroll ? 0.89 : 0.11), 'healthBar', function():Float { return health; }, 0, 2);
		...
		artist = Paths.getTextFromFile("MetaSongs/" + SONG.song + "meta");
	...
	{
		//FIX BY HONTON129, THANK ME LATER DANIELAPC!
		//Se desactivo esta version: no fijaba camara (camHUD), por eso no se veia bien en pantalla.
		//La version correcta (con camara fija) esta en create(), justo antes de crear el healthBar.
		//if (OpenFlAssets.exists(Paths.json(SONG.song.toLowerCase() + "/meta"), TEXT))
		//{
		//	var songTags:SongMetaTags = new SongMetaTags(0, 0, SONG.song);
		//	add(songTags);
		//}
	...
	override public function update(elapsed:Float)
	{
		...
		//Perdiendo
		if (healthBar.percent < 20) iconP1.animation.curAnim.curFrame = 1; //Icono de perder del player 1
		if (healthBar.percent > 80) iconP2.animation.curAnim.curFrame = 1; //Icono de Perder del player 2

		//Ganando (solo si el icono tiene ese 3er frame)
		if (iconP1.iconNumFrames > 2 && healthBar.percent > 85) iconP1.animation.curAnim.curFrame = 2; //WinIcon de Player 1
		if (iconP2.iconNumFrames > 2 && healthBar.percent < 15) iconP2.animation.curAnim.curFrame = 2; //WinIcon de player 2

        //por ahora solo codeare el ground y el sky  de la week 7
	...
	public function initHScript(file:String)
	{
		try
		{
			...
			{
				addTextToDebug('ERROR ON LOADING: ${newScript.parsingExceptions[0].message}', FlxColor.RED);
				newScript.destroy();
				return;
			}
